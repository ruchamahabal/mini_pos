...
const savedStarValue = computed(() => {
  const raw = (modelValue.value || 0) * nStars.value
...
function onStarClick(event, i) {
  ...
  const currentStarValue = Math.round((modelValue.value || 0) * nStars.value * 2) / 2
  if (newStarValue === currentStarValue) newStarValue = 0

  modelValue.value = newStarValue / nStars.value
